...
{                               /* implements figure 5A - draws lines parallel to ideal line */
   ...
   dist2 = (pt.x - murphy.last2.x) * (pt.x - murphy.last2.x)
       + (pt.y - murphy.last2.y) * (pt.y - murphy.last2.y);
...
#if 0
   if ((murphy.last1.x + murphy.last2.x) / 2 - p0.x >
       (murphy.first1.x + murphy.first2.x) / 2 - p0.x) {
      ml1b = ml1;
      ml2b = ml2;
   }
#endif
