...
/*		return SIZE;*/
  ...
  ValueMask = (unsigned long) ( CWBackPixel | CWBorderPixel | CWBackingStore );
  ...
  XWin = XCreateWindow (XDisplay, XRoot,
			...
			1, 0,
			CopyFromParent, (Visual*)CopyFromParent, ValueMask, &WinAttr);
      ...
      else if (WaitEvent.type == ButtonRelease)
	{
	  if (WaitEvent.xbutton.button == Button1)
	      ...
	      if (oversized)
		{
	      ...
	      XFillRectangle (XDisplay, XWin, XGcWin, 0, 0, (unsigned int)scr_width,
