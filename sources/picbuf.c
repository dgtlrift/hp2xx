...
static void swapout_RowBuf(RowBuf * row, const PicBuf * picbuf)
{
	...
	if ((int)
	    fwrite((char *) row->buf, picbuf->nb, picbuf->depth,
		   picbuf->sd)
	    != picbuf->depth) {
...
static void swapin_RowBuf(RowBuf * row, const PicBuf * picbuf)
{
	...
	if ((int)
	    fread((char *) row->buf, picbuf->nb, picbuf->depth, picbuf->sd)
	    != picbuf->depth) {
 ...
 **/
		...
		for (nr = 0; nr < pb->nr; nr++)
			if ((int)
			    fwrite((char *) pb->row[0].buf, pb->nb,
				   pb->depth, pb->sd)
			    != pb->depth) {
...
static void
line_PicBuf(DevPt * p0, DevPt * p1, PEN_W pensize, int pencolor,
...
void polygon_PicBuf(DevPt p4, DevPt p2, DevPt p1, DevPt p3, int pencolor,
		    PicBuf * pb)
{
