...
static void swapout_RowBuf(RowBuf * row, const PicBuf * picbuf)
{
   ...
   if ((int) fwrite((char *) row->buf, (size_t)picbuf->nb, (size_t)picbuf->depth, picbuf->sd)
       != picbuf->depth) {
...
static void swapin_RowBuf(RowBuf * row, const PicBuf * picbuf)
{
   ...
   if ((int) fread((char *) row->buf, (size_t)picbuf->nb, (size_t)picbuf->depth, picbuf->sd)
       != picbuf->depth) {
...
static void plot_RowBuf(RowBuf * rowbuf, int x, int depth, PEN_C color_index)
/**
 ...
 **/
      ...
      for (nr = 0; nr < pb->nr; nr++)
         if ((int) fwrite((char *) pb->row[0].buf, (size_t)pb->nb, (size_t)pb->depth, pb->sd)
             != pb->depth) {
...
static void dot_PicBuf(DevPt * p0, int pensize, PEN_C pencolor, PicBuf * pb)
{
...
static void
line_PicBuf(DevPt * p0, DevPt * p1, PEN_W pensize, PEN_C pencolor, int consecutive, const OUT_PAR * po)
    /**
...
void polygon_PicBuf(DevPt p4, DevPt p2, DevPt p1, DevPt p3, PEN_C pencolor, PicBuf * pb) {
...
/* coefficients for this edge */
         ...
         C2 = (double)(polygon[j].x * (polygon[j].y - polygon[j + 1].y) +
