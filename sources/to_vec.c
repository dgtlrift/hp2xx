...
HPGL_Pt         old_pt;
  ...
  else
  {
	...
	if (mode == 7 ) fprintf(md, scale_cmd, po->xoff,po->yoff, po->width+po->xoff,po->height+po->yoff);
		...
		if (mode == 3)  /* Needs special treatment: no polygons!        */
		{
			...
			switch(po->specials) {
				...
				case 2: /* pen width translates to color number */
					mapped_pen_no=(int)floor(pensize*10.);
					mapped_pen_size=pensize;
					if (mapped_pen_no < 1) mapped_pen_no =1;
					break;
				case 3:	/* as above, upper limit at 0.4 == 4 */	
					mapped_pen_no=(int)floor(pensize*10.);
					if (mapped_pen_no < 1) mapped_pen_no =1;
					if (mapped_pen_no > 1) mapped_pen_no =4;
					mapped_pen_size=pensize;
					break;
